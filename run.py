...
def exp_baselines(
    ...
    # embeddings = np.loadtxt('/Users/woodie/Desktop/workspace/Event-Series-Detection/resource/embeddings/10k.autoencoder.txt', delimiter=',')
    ...
    for N in retrieval_range:
        ...
        for e in range(epoches):
            ...
            # em.A  = init_em.A
            # ps, rs, _, _ = em.fit(T=t[-1], tau=t[0], iters=iters, first_N=N, specific_labels=specific_labels)
            p, r = utils.retrieval_test(m, l, specific_labels=specific_labels, first_N=N)
            print(p, r)
            precision.append(p)
            recall.append(r)
        precisions.append(precision)
        recalls.append(recall)
    # save exp results
    np.savetxt("result/svd1k_%s_precision_N_from%dto%d.txt" % (category, min(retrieval_range), max(retrieval_range)), precisions, delimiter=',')
    np.savetxt("result/svd1k_%s_recalls_N_from%dto%d.txt" % (category, min(retrieval_range), max(retrieval_range)), recalls, delimiter=',')
